...
class ForexEnv(TradingEnv):
    ...
    def _process_data(self):
        ...
        prices_open = self.df.loc[:,'Open'].to_numpy()
        prices_open = prices_open[self.frame_bound[0]-self.window_size:self.frame_bound[1]]

        diff = np.insert(np.diff(prices), 0, 0)
        
        signal_features = np.column_stack((prices, diff))

        return prices, prices_open, signal_features
    ...
    def _calculate_reward(self, action):
        ...
        if trade:
            ...
            current_price_open = self.prices_open[self._current_tick]
            last_trade_price_open = self.prices_open[self._last_trade_tick]
            
            #price_diff = current_price - last_trade_price

            if self._position == Positions.Short:
                step_reward += -(current_price_open - last_trade_price) * 10000
            elif self._position == Positions.Long:
                step_reward += (current_price - last_trade_price_open) * 10000
        ...
        if trade or self._done:
            ...
            if self.unit_side == 'left':
                if self._position == Positions.Short:
                    ...
                    self._total_profit = quantity / current_price_open

            elif self.unit_side == 'right':
                if self._position == Positions.Long:
                    quantity = self._total_profit / last_trade_price_open
