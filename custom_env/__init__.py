...
class ExternalDataFullFeature(ForexEnv):
    def _process_data(self):
        start = self.frame_bound[0] - self.window_size
        end = self.frame_bound[1]
        prices = self.df.loc[:, 'AskClose'].to_numpy()[start:end]
        signal_features = self.df.loc[:, ['AskOpen','AskHigh','AskLow','AskClose']].to_numpy()[start:end]
